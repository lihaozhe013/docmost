...
import { IconAt, IconColumns2, IconColumns3, IconMoodSmile, IconTable } from '@tabler/icons-react';
      ...
      <Menu shadow="md" position="bottom-start" withArrow={false}>
        <Menu.Target>
          <Tooltip label={t('Columns')} withArrow>
            <ActionIcon variant="subtle" color="dark" size="md" aria-label={t('Columns')}>
        ...
        <Menu.Dropdown>
          <Menu.Item
            ...
            onClick={() => editor.chain().focus().insertColumns({ layout: 'two_equal' }).run()}
          >
            {t('{{count}} Columns', { count: 2 })}
          </Menu.Item>
          <Menu.Item
            leftSection={<IconColumns3 size={16} />}
            onClick={() => editor.chain().focus().insertColumns({ layout: 'three_equal' }).run()}
          >
            {t('{{count}} Columns', { count: 3 })}
          </Menu.Item>
          <Menu.Item
            leftSection={<IconColumns4 size={16} />}
            onClick={() => editor.chain().focus().insertColumns({ layout: 'four_equal' }).run()}
          >
            {t('{{count}} Columns', { count: 4 })}
          </Menu.Item>
          <Menu.Item
            leftSection={<IconColumns5 size={16} />}
            onClick={() => editor.chain().focus().insertColumns({ layout: 'five_equal' }).run()}
          >
