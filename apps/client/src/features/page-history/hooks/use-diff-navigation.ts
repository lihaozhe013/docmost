 ...
 */
export function useDiffNavigation(scrollViewportRef: RefObject<HTMLDivElement>) {
  ...
  const handlePrevChange = useCallback(() => {
    ...
    const newIndex = currentChangeIndex <= 1 ? diffCounts.total : currentChangeIndex - 1;
  ...
  const handleNextChange = useCallback(() => {
    if (!diffCounts || diffCounts.total === 0) return;
    const newIndex = currentChangeIndex >= diffCounts.total ? 1 : currentChangeIndex + 1;
    setCurrentChangeIndex(newIndex);
    scrollToChangeIndex(newIndex);
  }, [diffCounts, currentChangeIndex, scrollToChangeIndex]);
