...
import { OAUTH_SCOPE_KEY, OAuthRouteScope } from '../decorators/oauth-scope.decorator';
...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  handleRequest(err: any, user: any, info: any, ctx: ExecutionContext) {
    ...
    const requiresSession = this.reflector.getAllAndOverride<boolean>(REQUIRE_SESSION_AUTH_KEY, [
      ctx.getHandler(),
      ctx.getClass()
    ]);
    if (requiresSession && user.authType !== JwtType.ACCESS) {
      ...
      throw new ForbiddenException('This action requires an interactive user session');
    }

    if (user.oauth) {
      const required = this.reflector.getAllAndOverride<OAuthRouteScope | undefined>(
        OAUTH_SCOPE_KEY,
  ...
  setJoinedWorkspacesCookie(user: any, ctx: ExecutionContext) {
    if (this.environmentService.isCloud()) {
      ...
      let workspaceIds = [];
      try {
        workspaceIds = req.cookies.joinedWorkspaces ? JSON.parse(req.cookies.joinedWorkspaces) : [];
      } catch (err) {
        /* empty */
      }
