import { InsertableUserToken, UpdatableUserToken, UserToken } from '@docmost/db/types/entity.types';
...
export class UserTokenRepo {
  ...
  async findById(token: string, workspaceId: string, trx?: KyselyTransaction): Promise<UserToken> {
    const db = dbOrTx(this.db, trx);

    return db
      .selectFrom('userTokens')
      .select(['id', 'token', 'userId', 'workspaceId', 'type', 'expiresAt', 'usedAt', 'createdAt'])
      .where('token', '=', token)
  ...
  async deleteExpiredUserTokens(trx?: KyselyTransaction): Promise<void> {
    const db = dbOrTx(this.db, trx);
    await db.deleteFrom('userTokens').where('expiresAt', '<', new Date()).execute();
  }
}
